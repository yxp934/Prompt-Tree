...
describe("ContextBoxService", () => {
  ...
  it("reads, writes, updates, and deletes context boxes", async () => {
    ...
    await service.put({
      id: "box-1",
      blocks: [{ id: "n1", kind: "node", nodeId: "n1" }],
      totalTokens: 12,
      maxTokens: 8192,
      createdAt: 1,
    });
    ...
    expect(loaded?.blocks).toEqual([{ id: "n1", kind: "node", nodeId: "n1" }]);

    const updated = await service.update("box-1", {
      blocks: [
        { id: "n1", kind: "node", nodeId: "n1" },
        { id: "n2", kind: "node", nodeId: "n2" },
      ],
      totalTokens: 25,
    });
    expect(updated.blocks.map((b) => b.id)).toEqual(["n1", "n2"]);
